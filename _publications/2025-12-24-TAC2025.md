...
<style>
  ...
  .cis-grid__cell {
    position: relative;
    width: 100%;
    /* Taller than 16:9 so Drive's player chrome (top bar + controls) has its
       own room and the video itself displays at its full size inside. */
    aspect-ratio: 4 / 3;
    /* Don't clip what's inside — Drive's player should size itself to fit. */
    overflow: visible;
  }
